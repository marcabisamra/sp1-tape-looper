...
/* SP-1 U1 (SOF-OFF). Set to 0 to restore stock SOF-driven behaviour.
 * With CONFIG_UDC_ENABLE_SOF=n, uac2_sof() never runs, so its
 * per-frame feedback write is gone. The feedback EP's own completion
 * re-arms it in the common case, but the `if (err) return 0;` path in
 * uac2_request() leaves that loop dead with nothing to restart it --
 * SOF used to be the restart. So we also re-arm from the ISO OUT
 * completion, which is the most reliable per-frame tick while audio
 * is streaming, using the SAME guards uac2_sof() used. */
#define SP1_U1_SOF_OFF 1
...
#if SP1_U1_SOF_OFF
		/* SP-1 U1: uac2_sof()'s per-frame feedback write, relocated
		 * here. Same cadence (one ISO OUT completion per frame while
		 * streaming) and the same three guards it used. */
		{
			const struct usb_ep_descriptor *sp1_fb_ep =
				get_as_feedback_ep(c_data, as_idx);

			if ((sp1_fb_ep != NULL) &&
			    !(ctx->fb_queued & ctx->fb_double & BIT(as_idx)) &&
			    atomic_test_bit(&ctx->as_active, as_idx)) {
				write_explicit_feedback(c_data,
					sp1_fb_ep->bEndpointAddress,
					cfg->as_terminals[as_idx]);
			}
		}
#endif
